...
function PostList({ onPostClick }) {
  ...
  useEffect(() => {
    if (postIdFilter) {
      setFilteredPosts(posts.filter((post) => post.id.toString() === postIdFilter));
    } else {
    ...
    <div className="post-list flex flex-col items-center justify-center bg-[#250c25] text-white p-8 ">
      ...
      <ul>
        {filteredPosts.map((post) => (
          <li key={post.id} onClick={() => handlePostClick(post.id)} className='my-10'>
            <strong>{post.id}. Post Title: {post.title}</strong>
